...
def get_path():
    ...
    if path == []:
        ...
        frag_dir = ipath + '/modules/frag_database/'
...
def make_binders(data, data2, keywords):
    ...
    for ornt in range(num_orient):
        ...
        for sstr in range(num_sstr):
            ...
            pdb_data = tc.calculate_rmsd(binders_data, data2, name_jdx, [str(ornt), str(sstr)], sstr_jdx.tolist(), keywords['prefix'], frag_dir, swap)
    ...
    for ornt in range(num_orient):
        ...
        for sstr in range(num_sstr):
            for mm in range(len(model_bb)):
                ...
                pdb_data = tc.calculate_rmsde(binders_data, data2, name_jdx, name_b_jdx, [str(ornt), str(sstr), str(mm)], sstr_jdx.tolist(), keywords['prefix'], frag_dir, e, swap, c_names)
                pdb_data_all += pdb_data
    return pdb_data_all
