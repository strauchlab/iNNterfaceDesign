...
def mutate_loop(data, dir, dir1, dir2, k1):
    ...
    for cx, cpx in enumerate(data[:]):
        name, seq = cpx[0], cpx[1] 
        pocket = pose_from_pdb(dir1 + name + 'pocket.pdb')
        loop1 = pose_from_pdb(dir1 + name + 'binder.pdb')
        ...
        for m in range(len(unq_seq)):
            ...
            name_m = name + pt_m + '_cpx.pdb'
            loop_rel.dump_pdb(dir3 + name_m)
            score_data.append([name_m, unq_seq[m], cpx_res[0][0], ddG])
